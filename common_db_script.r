...
pct <- function(x, total) {
    result = paste(x, "")
    while (nchar(result) < 15)
        result = paste(result, "")
    paste(result, round((x/total) * 100, 2), "%")
}
...
createTables <- function(prefix = "") {
    if (prefix != "") {
        table.projects <<- paste(prefix, "projects", sep="_")
        table.files <<- paste(prefix, "files", sep="_")
        table.filesTokens <<- paste(prefix, "filesTokens", sep="_")
        table.clonePairs <<- paste(prefix, "clonePairs", sep="_")
        table.cloneInfo <<- paste(prefix, "cloneInfo", sep="_")
        table.cloneGroups <<- paste(prefix, "cloneGroups", sep="_")
    } else {
        table.projects <<- "projects"
        table.files <<- "files"
        table.filesTokens <<- "filesTokens"
        table.clonePairs <<- "clonePairs"
        table.cloneInfo <<- "cloneInfo"
        table.cloneGroups <<- "cloneGroups"
    }
    sql.query("CREATE TABLE IF NOT EXISTS ",table.projects,"(
              id INT NOT NULL,
              path VARCHAR(1000) NOT NULL,
              url VARCHAR(1000) NOT NULL,
              PRIMARY KEY(id))")
    println(table.projects, " - project to path mapping")
    sql.query("CREATE TABLE IF NOT EXISTS ",table.files,"(
              ...
              sloc INT NOT NULL,
              PRIMARY KEY(id))")
    sql.query("CREATE TABLE IF NOT EXISTS ",table.filesTokens," (
              projectId INT NOT NULL, 
              id INT NOT NULL,
              totalTokens INT NOT NULL,
              uniqueTokens INT NOT NULL,
              PRIMARY KEY(id))")
    println(table.filesTokens, " - file token statistics - only files going to sourcererCC")
    sql.query("CREATE TABLE IF NOT EXISTS ", table.clonePairs, " (
              projectId1 INT NOT NULL,
              fileId1 INT NOT NULL,
              projectId2 INT NOT NULL,
              fileId2 INT NOT NULL,
              PRIMARY KEY(fileId1, fileId2))")
    println(table.clonePairs, " - clone pairs as reported by the tokenizer")
    sql.query("CREATE TABLE IF NOT EXISTS ", table.cloneInfo, "(
            fileId INT NOT NULL,
            groupId INT NOT NULL,
            projectId INT NOT NULL,
            PRIMARY KEY(fileId))")
    println(table.cloneInfo, " - clone group membership for files sourcererCC reported clones")
    sql.query("CREATE TABLE IF NOT EXISTS ", table.cloneGroups, "(
            groupId INT NOT NULL,
            files INT NOT NULL,
            projects INT NOT NULL,
            PRIMARY KEY(groupId))")
    println(table.cloneGroups, " - clone groups from sourcererCC reported clones")
...
loadDataFromFolder <- function(table, folder, overwrite = OVERWRITE, index = F) {
    println("Loading table ", table, " from disk folder ", folder)
    x <- list.files(folder, full.names = T)
    println("  ", length(x), " files found")
    if (length(x) == 0) {
        println("  no files -- skipping")
        return(invisible(NULL))
    }
    # find the newest file
    modified = max(as.integer(file.info(x)$mtime))
    if (sql.timestamp.shouldOverwrite(modified, table, overwrite)) {
        t = system.time({
            sql.query("DELETE FROM ", table)
            for (f in x) {
                println("    loading file ", f)
                sql.query("LOAD DATA LOCAL INFILE '", f, "' INTO TABLE ",table," FIELDS TERMINATED BY ',' ENCLOSED BY '\"'")
                println("      ", sql.affectedRows(), " entries in ", sql.lastTime(), " [s]")
            }
            if (index != F)
                println("      ", sql.createIndex(table, index))
        })
        println("  done in ", t[[3]], " [s]")
        sql.setTimestamp(table, modified)
    } else {
        println("  skipping loading table ", table, " - already loaded, timestamp ", timestamp.to.date(modified))
    }
    x <- sql.tableStatus(table, F)
    println("  ~", x$length, " entries [", x$bytes, " bytes]")
    invisible(NULL)
}

createFilesTable <- function(table, folder, overwrite = OVERWRITE, ignoreEmptyFiles = T, index = F) {
    println("Loading table ", table, " from disk folder ", folder, " and converting from js-tokenizer stats to sourcererCC format")
    x <- list.files(folder, full.names = T)
    println("  ", length(x), " files found")
    if (length(x) == 0) {
        println("  no files -- skipping")
    ...
    if (sql.timestamp.shouldOverwrite(modified, table, overwrite)) {
        t = system.time({
            sql.query("DELETE FROM ", table)
            sql.query("DROP TABLE IF EXISTS tmp_full_stats")
            sql.query("CREATE TABLE tmp_full_stats (
                id INT NOT NULL,
                projectId INT NOT NULL,
            ...
            for (f in x) {
                println("    loading file ", f)
                sql.query("LOAD DATA LOCAL INFILE '", f, "' INTO TABLE tmp_full_stats FIELDS TERMINATED BY ',' ENCLOSED BY '\"'")
                println("      ", sql.affectedRows(), " entries in ", sql.lastTime(), " [s]")
            }
            if (ignoreEmptyFiles) {
                println("    removing empty files")
                sql.query("DELETE FROM tmp_full_stats WHERE bytes = 0")
                println("      ", sql.affectedRows(), " entries in ", sql.lastTime(), "[s]")
            }
            println("    creating ", table, " table")
            sql.query("INSERT INTO ", table, " SELECT projectId, id, CONCAT(projectPath, ' ', relPath), url, fileHash, bytes, loc, (loc - emptyLoc), (loc - emptyLoc - commentLoc) FROM tmp_full_stats")
            println("      ", sql.affectedRows(), " entries in ", sql.lastTime(), " [s]")
            sql.query("DROP TABLE tmp_full_stats")
            if (index != F)
                println("      ", sql.createIndex(table, index))
        })
        println("  done in ", t[[3]], " [s]")
...
calculateCloneGroups <- function(prefix = "", overwrite = OVERWRITE) {
    if (prefix != "") {
        table.projects <<- paste(prefix, "projects", sep="_")
        table.files <<- paste(prefix, "files", sep="_")
        table.clonePairs <<- paste(prefix, "clonePairs", sep="_")
    ...
    println("Calculating clone groups from clone pairs in ", table.clonePairs)
    ts = sql.timestamp(table.clonePairs)
    if (sql.timestamp.shouldOverwrite(ts, table.cloneGroups, overwrite) || 
        sql.timestamp.shouldOverwrite(ts, table.cloneInfo, overwrite)) {
        sql.query("DROP TABLE IF EXISTS tmp_clones")
        sql.query("DROP TABLE IF EXISTS tmp_map")
        sql.query("DELETE FROM ", table.cloneInfo)
        sql.query("DELETE FROM ", table.cloneGroups)
        ...
        sql.query("CREATE TABLE tmp_clones SELECT fileId1, fileId2 FROM ", table.clonePairs)
        ...
        sql.query("INSERT INTO ", table.cloneInfo, " SELECT fileId1, MIN(fileId2), MIN(projectId1) FROM ", table.clonePairs, " GROUP BY fileId1")
        println("    ", sql.affectedRows(), " entries inserted in ", sql.lastTime(), " [s]")
        println("    ", sql.createIndex(table.cloneInfo, "fileid"));
        println("    ", sql.createIndex(table.cloneInfo, "groupId", F))
        sql.query("INSERT INTO ", table.cloneInfo, " SELECT fileId2, MIN(fileId1), MIN(projectId2) FROM ", table.clonePairs, " WHERE fileId2 NOT IN (SELECT fileId FROM ", table.cloneInfo, ") GROUP BY fileId2")
        println("  unifying clone groups")
        repeat {
            sql.query("INSERT INTO tmp_map SELECT fileId1, MIN(fileId2) FROM tmp_clones WHERE fileId1 IN (SELECT groupId FROM ", table.cloneInfo, ") GROUP BY fileId1");
            x <- sql.affectedRows();
            if (x == 0)
                break;
            ...
            repeat {
                sql.query("REPLACE INTO ", table.cloneInfo, " SELECT fileId, newId, projectId FROM ", table.cloneInfo, " INNER JOIN tmp_map ON groupId=oldId");
                x <- sql.affectedRows();
                if (x == 0)
                    break
        ...
        println("  creating clone groups summaries")
        sql.query("INSERT INTO ",table.cloneGroups, " (SELECT groupId, COUNT(fileId) AS files, COUNT(DISTINCT projectId) AS projects FROM ", table.cloneInfo, " GROUP BY groupId)");
        println("    ", sql.affectedRows(), " groups in ", sql.lastTime(), "[s]")

        sql.query("DROP TABLE tmp_map")
        sql.query("DROP TABLE tmp_clones")
        
        sql.setTimestamp(table.cloneGroups, ts)
        sql.setTimestamp(table.cloneInfo, ts)
    } else {
        println("  skipping clone groups preprocessing into ", table.cloneGroups," - already created, timestamp: ", timestamp.to.date(ts))
    }
    x <- sql.tableStatus(table.cloneInfo, F)
    println("  ~", x$length, " entries [", x$bytes, " bytes]")
    x <- sql.tableStatus(table.cloneGroups)
